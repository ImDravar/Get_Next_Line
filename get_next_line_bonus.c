...
/*   Updated: 2023/04/08 16:39:05 by rruiz-sa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (!box[fd] || (box[fd] && !ft_strchr(box[fd], '\n')))
		box[fd] = ft_read(fd, box[fd]);
	if (!box[fd])
		return (NULL);
	while (box[fd][i] && box[fd][i] != '\n')
		i++;
	line = ft_substr(box[fd], 0, i + 1);
	if (!line || !line[0])
		return (ft_free_double(&box[fd], &line));
	box[fd] = ft_cut(box[fd]);
	return (line);
}
